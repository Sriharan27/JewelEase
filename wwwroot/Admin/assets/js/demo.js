...
async function loadSalesDataForcast() {
	try {
		...
		for (const itemName in groupedSales) {
			...
			// Sort sales data by month
			itemSales.sort((a, b) => a.month - b.month);

			// Get sales quantities in order of months
			const quantities = itemSales.map(sale => sale.totalQty);

			// Calculate average monthly growth rate (basic AI/ML logic)
			const growthRates = [];
			...
			forecastData.push({
                ...
                category: categoryName // Add category to the dataset object
				...
				tooltips: {
					callbacks: {
						title: function (tooltipItem, data) {
							// Return the title as "Next Month"
							const dataset = data.datasets[tooltipItem[0].datasetIndex];
							return [dataset.category]; // Category name is set as the title
						},
						label: function (tooltipItem, data) {
							// Get the dataset index to find the corresponding category
							const dataset = data.datasets[tooltipItem.datasetIndex];
							const category = dataset.category;
							const itemName = dataset.label;

							// Create tooltip label with category and item name
